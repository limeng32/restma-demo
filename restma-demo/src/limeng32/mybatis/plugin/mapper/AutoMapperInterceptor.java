package limeng32.mybatis.plugin.mapper;
...
public class AutoMapperInterceptor implements Interceptor {
	private static final Log logger = LogFactory
			.getLog(AutoMapperInterceptor.class);
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (null == originalSql || "".equals(originalSql)) {
			...
			case "select":
				...
				break;
			case "selectAll":
				newSql = SqlBuilder.buildSelectAllSql(mappedStatement
						.getResultMaps().get(0).getType());
				break;
			case "update":
			...
			List<ParameterMapping> parameterMappings = sqlSource.getBoundSql(
					parameterObject).getParameterMappings();
			metaStatementHandler.setValue("delegate.boundSql.sql", sqlSource
					.getBoundSql(parameterObject).getSql());
			metaStatementHandler.setValue(
					"delegate.boundSql.parameterMappings", parameterMappings);
		}
		// 调用原始statementHandler的prepare方法完成原本的逻辑
		statementHandler = (StatementHandler) metaStatementHandler
				.getOriginalObject();
